...
df3_5 <- read_csv("hybrid_by_week_cleaned_weather.csv",col_types = cols("Repl" = col_integer(), "rainMin" = col_number(), "rainMax" = col_number(), "rainMean" = col_number(), "rainMedian" = col_number(), "solarMin" = col_number(), "solarMax" = col_number(), "solarMedian" = col_number(), "windDirMin" = col_number(), "windDirMax" = col_number(), "windDirMedian" = col_number()))

setwd("hyb_wks3-5_by_week_1k_bin/")
hyb <- readRDS("output/riskProfObj.rda")

s3_5 <- df3_5 %>% filter(Week >= isoweek(Planted)+3 & Week <= isoweek(Planted)+5)
val <- grep("Min|Max",names(s3_5))
numericVars <- names(s3_5[val])[vapply(s3_5[val], function(x) var(x) != 0, logical(1))]
hyb1 <- data.frame(optcl = hyb$riskProfClusObj$clustering,Exp = s3_5$Exp, Hyb = s3_5$Pedi, Yield = s3_5$Yield, s3_5[numericVars])
