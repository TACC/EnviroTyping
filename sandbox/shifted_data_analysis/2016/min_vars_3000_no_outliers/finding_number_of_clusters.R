...
while (any(clusObj$clusterSizes <= 3 )) {
    ...
    set.seed(1234)
    runInfoObj <- profRegr(covNames, outcome = 'yield', yModel = 'Normal', xModel = "Mixed", discreteCovs = "Pedi", continuousCovs = max_vars, data = update, nSweeps = 3000, nBurn = 50, nProgress = 100)
    calcDists <- calcDissimilarityMatrix(runInfoObj)
    clusObj <- calcOptimalClustering(calcDists)
...
#set.seed(1234)
#runInfoObj <- profRegr(covNames, outcome = 'Yield', yModel = 'Normal', xModel = "Mixed", discreteCovs = "Pedi", continuousCovs = max_vars, data = clus_3, nSweeps = 3000, nBurn = 50, nProgress = 100)
#calcDists <- calcDissimilarityMatrix(runInfoObj)
#clusObj <- calcOptimalClustering(calcDists)