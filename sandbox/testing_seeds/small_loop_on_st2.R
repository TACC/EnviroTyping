...
for (i in seed) {
    ...
    dir.create(dir_name)
    setwd(dir_name)
    runInfoObj <- profRegr(covNames, outcome = 'yield', yModel = 'Normal', xModel = "Mixed", discreteCovs = "pedi", continuousCovs = min_vars, data = df, nSweeps = 20, nBurn = 10, nProgress = 1, seed = i)
